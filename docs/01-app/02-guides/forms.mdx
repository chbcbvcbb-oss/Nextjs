...
You can then conditionally render the error message based on the `state` object. The shape of the `state` object will match the shape that is returned by your server action.

```tsx filename="app/ui/signup.tsx" highlight={13,20-22} switcher
'use client'
...
```jsx filename="app/ui/signup.js" highlight={13,20-22} switcher
'use client'
