...
The scheduler prioritizes likely navigations while minimizing unused downloads. Pending prefetches resume through the same queue when the app recovers from an offline period; see [Handling connectivity drops](/docs/app/guides/offline-support).

### Cache Components

With [Cache Components](/docs/app/getting-started/caching) enabled, the prefetching strategy uses an [App Shell](/docs/app/glossary#app-shell) to make client navigations instant:
