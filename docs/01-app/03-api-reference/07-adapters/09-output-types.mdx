...
```typescript
{
  ...
  kind?: 'blocking' | 'fallback' | 'shell' | 'page'  // Point-in-time classification of what this build's prerender produced (revalidation can change it). blocking: no shell is prerenderable (blocking/omitted templates, or an app shell that is empty because everything postponed). fallback: a shell with pending UI that can still be prerendered into something more specific. shell: the most specific prerender possible; its pending UI resolves at request time (on the server when postponed, otherwise on the client). page: complete HTML with no pending UI. undefined: unclassified (route handlers)
  postponed?: boolean     // Whether this build's prerender postponed (React suspended on dynamic data), so serving it resumes on the server (per-request compute). Snapshot of the build artifact, not intrinsic to the route; false: prerendered without postponing, undefined: does not apply (pages router, route handlers, blocking templates)
